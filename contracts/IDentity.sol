...
contract Identity is Ownable {
    ...
    mapping(uint256 => VCStatus) private _vcId_to_VC;
    ...
    function add_user (
        ...
        require(_vcId_to_VC[_vc_id].credentialOwner == address(0), "VC has already a VC owner: request already stored, remember to activate it");
        ...
        _vcId_to_VC[_vc_id] = VCStatus(extractedAddress, _issuanceDate, _expirationDate, false);
        ...
        emit NewVCRequestRegistered(_vc_id, _vcId_to_VC[_vc_id].credentialOwner, _vcId_to_VC[_vc_id].expirationDate, block.timestamp);
    }

    function revokeVC(uint256 _vc_id) public onlyOwner {
        VCStatus storage vc = _vcId_to_VC[_vc_id];
    ...
    function _vc_expired(uint256 _vc_id) internal view returns(bool) {
        ...
        return block.timestamp > _vcId_to_VC[_vc_id].expirationDate;
    }
